...
int main(int argc, char **argv) {
    ThreadPool tp(argc > 1 ? stoi(argv[1]) : 10);
    ...
    for (int i = 0; i < futures.size(); i++) {
        ...
        cout << "Result " << i << ":" << result.toString() << endl;
    }
    ...
    // Show execution time
    cout << "Total time: "
            << chrono::duration_cast<chrono::milliseconds>(chrono::system_clock::now() - start_time).count()
            << " ms" << endl;
    return 0;
}
